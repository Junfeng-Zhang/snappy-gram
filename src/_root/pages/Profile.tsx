          ...
          <div className="flex justify-center gap-4">
            <div className={`${user.id !== currentUser.$id && "hidden"}`}>
              <Link
                ...
                <p className="flex whitespace-nowrap small-medium"> 修改信息 </p>
              </Link>
            </div>
            <div className={`${user.id === id && "hidden"}`}>
              <Button type="button" className="px-8 shad-button_primary"> 关注 </Button>
            </div>
          </div>
        </div>
      </div>
      ...
      {currentUser.$id === user.id && (
        <div className="flex w-full max-w-5xl">
          <Link
            ...
            <img src={"/assets/icons/posts.svg"} alt="posts" width={20} height={20} /> 帖子
          </Link>
          <Link
            to={`/profile/${id}/liked-posts`}
            className={`profile-tab rounded-r-lg ${pathname === `/profile/${id}/liked-posts` && "!bg-dark-3"
              }`}>
            <img src={"/assets/icons/like.svg"} alt="like" width={20} height={20} /> 点赞
          </Link>
        </div>
      )}
